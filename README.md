...
## docker-compose
- Adds `frontend` service with `VITE_API_URL=http://ai-backend:5000` and port `5173:5173`.
...
## AWS Notes
- Works on EC2 with the same compose file; open 5173/5000/8000 or put behind a reverse proxy/ALB. Keep 11434 internal if possible.
- For ECS, build/push images for `frontend`, `backend`, and `ai_service`. Use `ollama/ollama` for LLM. Keep service names and envs the same for internal networking.
